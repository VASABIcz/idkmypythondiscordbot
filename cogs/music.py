...
class music(commands.Cog):
    ...
    async def validate(self, ctx, opt):
        ydl = YoutubeDL(opt)
        ...
        if loljs[ctx.guild.id]['que']:
            for n in range(len(loljs[ctx.guild.id]['que'])):
                if await self.scrap(loljs[ctx.guild.id]['que'][n]['URL']):
            ...
            if filee != {}:
                for n in range(len(filee)):
                    ...
                    for i in range(len(filee[lil[n]])):
                        if await self.scrap(filee[lil[n]][i]['URL']):
                            loop = asyncio.get_event_loop()
    ...
    async def embed_crp(self, gid, play):
    ...
    async def embed_que(self, gid, nam):
        qee = loljs[gid]['que']
        ...
        if payload.user_id != self.bot.user.id:
                ...
                if payload.message_id == loljs[gid]['rpm']['mid']:
                    ...
                    if payload.emoji.name == '⏸️':
                        ...
                        try:
                            ...
                            await message.edit(embed=await self.embed_crp(gid, 'paused'))
                        except:
                            pass
                    if payload.emoji.name == '▶️':
                        guild = self.bot.get_guild(gid)
            ...
            if loljs[gid]['quem']['mid'] is not None:
                if payload.message_id == loljs[gid]['quem']['mid']:
                    ...
                    await message.edit(embed=await self.embed_que(gid, nam))
